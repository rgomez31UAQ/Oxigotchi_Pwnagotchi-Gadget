...
///
/// Returns an error string on failure instead of panicking — critical for
/// reliability on the Pi where GPIO/SPI access can fail transiently.
#[cfg(target_arch = "aarch64")]
pub fn flush_to_hardware(fb: &FrameBuffer, _config: &DisplayConfig) -> Result<(), String> {
    use std::thread;
    ...
    // Helper: send command
    let send_command =
        |spi: &mut Spi, dc: &mut rppal::gpio::OutputPin, cmd: u8| -> Result<(), String> {
            dc.set_low();
            spi.write(&[cmd])
    ...
    // Helper: send data
    let send_data =
        |spi: &mut Spi, dc: &mut rppal::gpio::OutputPin, data: &[u8]| -> Result<(), String> {
            dc.set_high();
            spi.write(data)
                .map_err(|e| format!("SPI write data ({} bytes): {e}", data.len()))?;
            Ok(())
        };

    // Helper: wait until BUSY pin goes low (with timeout)
    let wait_busy = |busy: &rppal::gpio::InputPin| -> Result<(), String> {
        let start = std::time::Instant::now();
        let timeout = Duration::from_secs(5);
        while busy.is_high() {
            if start.elapsed() > timeout {
                return Err("EPD BUSY timeout (5s)".into());
            }
