...
- **XP & leveling** — The bull earns XP passively (+1 per epoch just for scanning, +1 per AP seen) and actively (+100 per handshake capture). An exponential curve (`level^1.05 × 5`) makes early levels fast and high levels a grind — max level 999 takes about 7 months of daily use (8h/day with ~16 APs visible). XP persists across reboots. Handshakes accelerate leveling significantly.
- **Channel hopping** — Default channels are 1, 6, 11 (the non-overlapping 2.4GHz channels). AO hops between them sequentially, dwelling on each for 5 seconds before moving to the next. The single radio can only be on one channel at a time, so the dwell time is a tradeoff: shorter dwell = faster coverage but more firmware stress from frequent channel switches. **Keep the dwell time at 5 seconds or above** — the BCM43436B0 firmware is fragile around rapid channel switching, and dwell times under 3 seconds can trigger firmware crashes, especially with 4+ channels. Stick with 1,6,11 at dwell 5 for the best stability-to-coverage ratio.
...
Your bull earns XP three ways: **+1 per epoch** (passive, just for being on), **+1 per AP seen** (scales with how many networks are nearby), and **+100 per handshake** captured. With ~16 APs visible, that's ~17 XP every 30 seconds just from scanning. The level formula is exponential: `XP needed = level^1.05 × 5`. Early levels fly by (Lv 1 needs 5 XP, Lv 10 needs 56 XP), but high levels are a grind (Lv 500 needs 3,900 XP, Lv 999 needs ~18,000 XP per level). Max level is **999** — reaching it takes roughly **7 months** of daily use (8 hours/day, ~16 APs). Capturing handshakes significantly speeds this up. XP persists across reboots.
